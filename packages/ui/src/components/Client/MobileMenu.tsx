...
export function MobileMenu({ children }: PropsWithChildren<{}>) {
  ...
  return (
    <Transition show={isOpen} as={React.Fragment}>
      <Dialog as="div" className="lg:hidden" onClose={setIsOpen}>
        <TransitionChild
          as={React.Fragment}
          ...
          leaveTo="opacity-0"
        >
          <DialogPanel className="fixed inset-0 z-20 flex flex-col bg-white">
            {children}
          </DialogPanel>
        </TransitionChild>
      </Dialog>
...
export function MobileMenuLink({
  ...
  return (
    <DisclosureButton
      as="a"
      href={href}
      className="copy-medium-bold text-gray-dark hover:bg-gray-lightest block rounded-lg py-2 pl-6 pr-3 leading-7"
    >
      {title}
    </DisclosureButton>
  );
    ...
    <Disclosure as="div" className="">
      {({ open }) => (
        <div
          className={clsx(
            '',
            open && nestLevel === 2 && 'bg-gray-lighter mx-8 rounded',
          )}
        >
          <DisclosureButton
            className={clsx(
              'flex w-full items-center justify-between py-4 leading-5',
              open && 'text-kls-orange-primary',
              !open && 'text-gray-dark',
              open && nestLevel === 1 && 'bg-kls-orange-bright-alt',
              !open &&
                nestLevel === 1 &&
                'border-b-kls-orange-bright-alt border-b',
              nestLevel === 1 && 'copy-bold px-8',
